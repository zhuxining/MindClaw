...
**背景**：Agent 记忆会影响后续判断和行动。用户需要查看 Agent 记住了什么、这些记忆来自哪里、是否仍然有效，并把稳定内容升级为经验或共有知识。

**目标**：定义 Agent Memory 工作域的查看、分类、确认、修正、删除、降权和升级行为。
...
Given 记忆状态为待确认
When 用户点击 Confirm
Then 该记忆状态变为已确认，并记录确认时间

Given 用户点击 Edit
When 用户修改记忆内容并保存
Then 该记忆显示修改后内容，并记录用户修正来源
...
作为用户，我希望把稳定记忆升级为经验教训或共有知识，以便让可复用内容离开黑盒记忆。

**验收标准**：

Given 记忆状态为已确认
When 用户点击 Promote
Then 系统展示升级选项：经验教训、共有知识草稿

Given 用户选择经验教训
When 操作完成
Then 系统生成经验教训候选，并关联原记忆

Given 用户选择共有知识草稿
When 操作完成
Then 中央内容区打开知识草稿，包含标题、`tags`、`overview` 和正文

- [ ] 升级动作生成演化记录。
- [ ] 升级为共有知识后，原记忆归属显示为 `shared`。
- [ ] 未确认记忆不能升级为共有知识。
