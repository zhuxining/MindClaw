...
所有入口都必须回到同一个闭环：**输入 → 整理 → 草稿/行动 → 用户审阅 → 记忆与演化记录 → 经验教训候选 → 共有知识 → 再调用**。异步观察是这个闭环的旁路信号源，不是所有内容沉淀的必经阶段。
...
Agent 知识沉淀不另起一个不可读黑盒。凡是会影响长期行为、判断和建议的内容，默认进入可审阅空间；确实不能公开的内容，只能作为私有观察保留，并受更短生命周期和更强清理规则约束。
...
Markdown 是已确认知识内容的真相源。SQLite 负责索引、运行状态、Agent 记忆、演化记录和审核流程；向量索引、会话摘要都是可重建的派生层或运行态。
...
- SQLite 存索引、运行态、偏好、Agent 记忆、演化记录和待审核草稿
- 向量索引可从 Markdown 重建
- 私密区路径由后端强制隔离
...
| 概览层 | `overview` | 用少量文本说明笔记核心内容 | 判断是否需要打开正文 |
...
`overview` 不是摘要装饰，而是 Agent 的预读入口；`tags` 不是复杂分类系统，而是轻量知识路由入口。正文不是默认全量注入上下文，而是在索引层和概览层判断相关后再加载。
...
Agent 记忆不是知识库，也不是聊天记录。它是 Agent 为了后续判断和行动而维护的内部上下文。
...
- 未经确认的观察只能保持为假设
- 稳定、可复用、被确认的记忆可以生成经验教训候选；确认后进入共有知识，记忆只保留引用
...
反思/回顾输出的是建议和候选。只有经过用户确认或后续验证的内容，才能写入稳定记忆、生成演化记录，或进入共有知识库。
...
| 失败被修复 | 修复方法是否可复用，是否改变后续工作方式 | `LessonDraft` + 可选 `EvolutionLog` | 待审核经验教训 |
| 任务结束 | 本次任务是否产生偏好变化、失败修正、可复用流程 | 轻量回顾结果 | 记忆建议、经验教训候选 |
| 周期回顾 | 多次事件中是否出现重复模式 | `pattern` 记忆、`LessonDraft`、降权或删除建议 | 回顾面板中的批量审核项 |
...
Agent 使用这些结果时必须遵守两个边界：未确认观察只能作为低置信度参考；已确认知识和经验教训必须优先于未确认记忆。
...
| 状态 | 存储位置 | 含义 |
|------|----------|------|
| 候选态 | 结构化存储中的待审核草稿 | 从记忆、执行失败、用户反馈或演化记录中生成，允许编辑、拒绝、合并 |
...
Agent 记忆、演化记录、经验教训候选和共有知识承担不同职责，必须分开存储和审核。

| 对象 | 存储位置 | 用户操作 | 写入规则 |
|------|----------|----------|----------|
| Agent 记忆 | SQLite / MemoryStore | 查看、修正、遗忘、降权、确认 | 作为行动上下文使用，不直接等同于长期知识 |
| 演化记录 | SQLite / EvolutionLog | 查看、补充备注、标记错误 | 以追加为主，保留 Agent 判断和策略变化的证据链 |
| 经验教训候选 | SQLite / LessonDraft | 审核、编辑、拒绝、合并、确认 | 未确认前不进入共有知识库 |
| 确认后的经验教训 | Markdown 共有知识库 | 像普通知识文档一样编辑、重组、引用 | 承载共同真相，Agent 记忆只保留索引和引用 |
...
这意味着 SQLite 管理状态、审核流程和证据链；Markdown 管理确认后的共同知识。用户可以修改记忆和候选，但演化记录默认保留原始事实，只允许追加备注、纠错标记或关联新的修正记录。
...
2. Daily / Inbox / Vault 是否能形成稳定输入流
