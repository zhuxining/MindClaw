...
作为用户，我希望在一次协作结束后触发回顾，以便确认本次过程是否产生记忆、经验或知识。

**验收标准**：

Given Agent Session 中存在执行结果、用户纠正或失败记录
When 用户点击 Review Session
Then 系统打开回顾视图，列出待确认观察、记忆更新建议和可沉淀草稿

Given 回顾视图已打开
When 用户确认某条记忆更新建议
Then 该建议进入 Agent Memory 的待审阅或已确认状态

Given 用户确认某条经验沉淀建议
When 操作完成
Then 系统生成演化记录，并提供创建知识笔记的入口
