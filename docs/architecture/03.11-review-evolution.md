...
**Inbox 承载审核候选**：观察候选、记忆更新建议和经验教训候选先以 Inbox Markdown 保存；确认后的记忆、演化记录和经验归档再进入 `agent/`。
...
关键属性：Inbox ContextURI、观察摘要、来源、置信状态、适用范围、拒绝原因、归档去向。
...
关键属性：ContextURI、变化类型、变化前后内容、触发原因、来源、处理人备注、文档路径。

**LessonCandidate**：可复用经验教训的待审核候选。
关键属性：Inbox ContextURI、适用场景、规则、反例、触发条件、来源记录、归档去向。
...
| 失败被修复 | 修复方法是否可复用 | Inbox `LessonCandidate` Markdown | 确认后保存为知识 |
...
用户审核
    ...
    ├── 经验候选确认 -> Vault 主题位置或 agent/lessons -> Inbox 目标引用
...
| 候选是否使用统一 ContextIndex？ | 是，ReviewQueueIndex 只做 Inbox 队列视图 | 每类候选一张独立业务表 | 回顾项需要跨观察、建议、经验和演化记录统一排序和引用 |
