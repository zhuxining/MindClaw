...
/// Channel Gateway 抽象层
///
/// 定义消息渠道的统一接口。每个消息渠道（飞书、钉钉、企微等）实现 `ChannelGateway` trait，
/// 通过 `GatewayRegistry` 注册后即可接入 MessageBus 进行消息路由。
pub mod registry;
...
// ── CredentialsManager trait ────────────────────────────────

/// 凭证管理器 trait
///
/// 每个渠道自行管理其凭证生命周期（Token 刷新、存储等）。
/// 飞书的 `TokenManager` 是此 trait 的一个实现。
#[allow(dead_code)]
