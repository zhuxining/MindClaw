...
**MemoryVaultRef**：记忆对 Vault 笔记的引用。
关键属性：Vault 笔记路径、引用原因、触发条件、短摘要。
...
召回结果来自 ContextIndex，必须携带 ContextURI、来源、状态和 `confidence`，供 `ContextPipeline` 决定是否注入、摘要或仅提示可打开。用户修改记忆置信度后，MemoryService 必须更新 Frontmatter 并触发 ContextIndex 重建或增量同步。
