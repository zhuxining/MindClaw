...
Memory 子系统负责维护 Agent 可召回的结构化记忆、记忆状态和知识引用；不负责旁路观察、经验教训提炼、共有知识正文写入或 Runtime 可观测性。
...
**MemoryRecord**：Agent 为后续判断和行动保存的结构化记忆。
关键属性：归属、类型、状态、摘要、触发条件、来源、可信状态、更新时间。
...
- `Deleted` 不再影响 Agent 行动，但演化记录保留删除原因。
...
用户通过 UI 或对话明确要求 Agent 记住某个背景、偏好或约束时，系统生成 `MemoryRecord(Proposed)` 或直接进入用户确认流程。

### 路径 B：回顾建议确认

`Review & Evolution` 子系统生成 `MemoryUpdateProposal`。只有用户确认后，Memory 子系统才执行新增、修正、删除或降权。
...
召回结果必须携带来源和状态，供 `ContextPipeline` 决定是否注入、摘要或仅提示可打开。
...
| `Review & Evolution` | 生成记忆更新建议，接收记忆变更后的演化记录 |
| `NoteService` | 提供共有知识笔记读写；Memory 只保存引用 |
| `Storage` | 持久化结构化记忆、来源和知识引用 |
...
| 决策问题 | 选择 | 放弃的替代方案 | 理由 |
|---------|------|--------------|------|
| 经验教训正文是否写入 Memory？ | 否，只写 Markdown 知识，Memory 保留引用 | 将经验教训正文复制进 Memory | 避免黑盒记忆成为长期知识真相源 |
| Agent 是否能绕过审核直接写稳定记忆？ | 否，生成建议或待确认记录 | LLM 工具直接写 confirmed memory | 记忆会影响后续行动，必须可审阅 |
