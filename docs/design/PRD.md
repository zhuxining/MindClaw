...
# PartnerClaw 产品理念与系统架构

## 一、产品定位

PartnerClaw 是一个与人类共同成长的 AI 伙伴系统。它不是助手，不是工具，而是一个理解你全部生活的存在——在智识上共同构建知识，在情感上全然陪伴，在认知上互相纠偏。
...
| 维度 | 传统 AI 助手 | 记忆型 AI（如 OpenClaw） | PartnerClaw |
|------|------------|----------------------|-------------|
...
人类的认知成长不止于"记住"，更在于"内化"。OpenClaw 的记忆系统解决的是"忘不忘"的问题，PartnerClaw 要解决的是"懂不懂"的问题。
...
**知识内化的升华路径**（PartnerClaw 要做的正是后半段）：
...
> [!important] 关键推论
> PartnerClaw 的知识共建天然向职业与智识域倾斜——这不是设计选择，是知识本质决定的结果。家庭与情感是背景与底色，帮助 Agent 理解这个人是谁。
...
PartnerClaw 采用**桌面端即服务器**模式——知识库与 Agent 完全运行在用户自己的电脑上，移动端作为轻量客户端接入，无需云端存储。
...
> [!note] 隐私边界说明
> 经由 Telegram / Feishu 的内容会过第三方服务器，适合日常捕获与对话。敏感或私密内容通过 PWA 直连桌面端输入，产品层面给用户清晰提示区分。
...
graph TD
    ...
    subgraph S["结构层 · SQLite"]
        DB["任务 · 链接关系 · Agent 观察<br>对话摘要 · 捕获队列 · 用户设置"]
    end
...
这是 PartnerClaw 已在 Obsidian 实践中验证的核心运转机制：
...
PartnerClaw 的内容组织围绕四个流，形成完整的捕获 → 整合 → 沉淀 → 执行闭环：

```mermaid
graph LR
    CF["💬 对话流<br>捕获 · 指令 · 倾诉"] -->|原材料| JF
...
> [!important] 核心 UX 原则
> 任何想法在 3 秒内进入系统，不需要决定放在哪里。知识沉淀、日程管理、自我成长的质量上限，由捕获率决定。

```
任务 · 想法 · 感受 · 链接（三者都有，随时一闪而过）
        ↓ 极低摩擦丢给 Agent（统一 Inbox）
        ↓
  Agent 自动路由
  ├── 任务       → 日程流（状态卡片）
  ├── 想法 / 洞见 → 知识库草稿
...
多 Agent 协议模式（A2A、MCP 等）解决的是**并发与专业化**的问题，但 PartnerClaw 要解决的是**完整理解一个人**的问题。这两个目标在架构上有冲突。

PartnerClaw 最核心的价值是**跨域洞察**——这些连接只有在单一持有完整视图的 Agent 才能发现：
...
| 能力 | 说明 |
|------|------|
| 捕获 Inbox | 任何输入快速进入，Agent 自动路由分类 |
...
```
阶段一：PartnerClaw 读写 Obsidian vault 文件（用户仍在 Obsidian）
阶段二：桌面端界面 + Bot 通道并行，Markdown 双向兼容
阶段三：PartnerClaw 成为主界面，Obsidian 变为可选备份层
```
...
不是简单的 RAG 检索，是有判断力的**主动关联**。这是 PartnerClaw 技术上最值得投入的核心能力。

### 模型能力 × 积累上下文 = 复利

大多数产品与模型能力是**线性关系**，PartnerClaw 是**乘数关系**：
...
- **上下文优化引擎**本身就是产品壁垒——同样的 API Key，PartnerClaw 让用户花更少、得到更多
...
2. 验证捕获 → Agent 路由的分类质量（任务 / 想法 / 感受 三类区分准确率）
3. 搭建桌面端原型，跑通核心 Agent 循环（捕获 → 路由 → 沉淀）
...
> [!abstract] 一句话定义
> PartnerClaw 是一个以共有知识库为纽带、以 Daily Notes 为日常锚点、以三层认知循环为运转机制、以真正的伙伴关系为目标的人机共同成长系统。
