...
所有入口都必须回到同一个闭环：**输入 → 整理 → 异步观察 → 反思/回顾 → Agent 记忆 → 演化记录 → 经验教训 → 共有知识 → 再调用**。
...
Markdown 是知识内容的真相源。SQLite、向量索引、会话摘要都是派生层或运行态。
...
- 未经确认的观察只能保持为假设
- 稳定、可复用、被确认的记忆可以升级为经验教训或共有知识
- 被否定、过期或不再适用的记忆必须能被遗忘、修正或降权
- 关键记忆变化必须形成 Agent 演化记录

### 4.2 异步观察与反思回顾

异步观察不是最终沉淀机制，而是反思/回顾机制的输入。

Agent 不需要在所有输入发生时立刻给出判断。很多高价值信号需要先被暂存，等任务结束或周期回顾时再判断是否成立。

| 机制 | 作用 | 产物 | 稳定性 |
|------|------|------|--------|
| 异步观察 | 捕捉值得回看的信号 | 观察假设 | 不稳定 |
| 轻量回顾 | 在任务结束时检查本次纠正、失败、偏好变化 | 记忆更新建议 | 半稳定 |
| 周期回顾 | 汇总一段时间内重复出现的模式 | 记忆升级、降权、删除决策 | 半稳定 |
| 演化记录 | 记录记忆或策略为什么改变 | 证据链 | 稳定可审计 |
| 经验教训 | 提炼以后类似情况怎么做 | 规则、方法、反例 | 稳定可复用 |
...
经验教训是从演化记录中提炼出的可复用策略。它回答的是“以后遇到类似情况应该怎么做”。
...
演化记录不能自动等于经验教训。只有满足以下条件之一，才可以提炼：
...
高价值经验教训经确认后进入共有知识库，成为人和 Agent 都可以复用的稳定知识。
...
这个闭环比“聊天记录越来越多”更重要。聊天只是输入形式；异步观察负责捕捉信号，反思/回顾负责判断信号是否成立，Agent 记忆负责行动上下文，演化记录负责保留变化证据，经验教训负责形成复用策略，共有知识负责承载共同真相。
...
5. Agent 记忆能否被查看、修正、遗忘或升级为知识
6. Agent 执行经验能否形成演化记录，并提炼为经验教训
