...
Agent         ──► knowledge / daily / task（原生工具） ──► Services ──► Storage
                                                      ──► Memory   ──► Storage
```

> Agent 通过独立原生工具（`knowledge`、`daily`、`task`）访问 Services，每个工具对应一个 Service，JSON Schema 类型安全，由 ToolRegistry 管理，详见 [03.03-tools.md](./03.03-tools.md)。
